...
def ingest_bytes(
    *,
    ...
    try:
        ...
        log.info(
            "document_ready document_id=%s chunks=%s",
            document_id,
            len(chunks),
        )
...
def _extract_table(
    ...
    log.info(
        "csv_profile filename=%s rows=%s cols=%s",
        filename,
        profile["row_count"],
        profile["columns"],
    )
...
def _chunk_csv(
    ...
    schema = (
        f"{extracted['file_type'].upper()} file "
        f"{filename} with {profile['row_count']} rows. "
        f"Columns: {', '.join(profile['columns'])}. "
        f"Types: {json.dumps(profile['dtypes'])}. "
        "Numeric columns: "
        f"{', '.join(profile['numeric_columns']) or 'none'}."
    )
    ...
    for column in profile["numeric_columns"]:
        ...
        text = (
            f"{filename} column {column}: "
            f"min={series.min()}, "
            f"max={series.max()}, "
            f"mean={series.mean()}, "
            f"nulls={int(series.isna().sum())}"
        )
    ...
    entity_col = (
        profile["entity_columns"][0]
        if profile["entity_columns"]
        else None
    )

    year_col = (
        profile["year_columns"][0]
        if profile["year_columns"]
        else None
    )
        ...
        for (entity, year), group in grouped:
            ...
            for column in value_cols:
                ...
                if numeric.notna().any():
                    parts.append(
                        f"{column}="
                        f"{numeric.sum() if len(numeric.dropna()) > 1 else numeric.dropna().iloc[0]}"
                    )
            ...
            row_start = (
                int(group.index.min()) + 2
            )

            row_end = (
                int(group.index.max()) + 2
            )
...
def _chunk_metadata(
    ...
    return {
        ...
        "end_line": int(
            chunk.get("end_line") or 0
        ),
        "row_start": int(
            chunk.get("row_start") or 0
        ),
        "row_end": int(
            chunk.get("row_end") or 0
        ),
